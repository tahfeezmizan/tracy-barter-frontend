...
import {
  useGetPastOrdersHistoryQuery,
  useSendBookingChatMutation,
} from "@/redux/features/AIforGrocery/AIforGrocery";
    ...
    if (open && pastOrdersHistory?.data?.length > 0 && !hasLoadedHistory) {
      ...
      if (historyData?.conversationHistory?.length > 0) {
        const mappedHistory: ChatMessage[] =
          historyData.conversationHistory.map((msg: any) => ({
            id: msg._id,
            role: msg.role,
            content: msg.content,
                  ...
                  message.role === "assistant" && (
                    <div className="mt-3 pt-3 border-t border-slate-200 space-y-2">
                      {message.suggestedItems.map((item, idx) => (
                        <div
                          key={idx}
                          className="flex items-center justify-between gap-2 p-2 bg-slate-50 rounded border border-slate-100"
                        >
